  ...
  <div class="fixed top-4 left-1/2 -translate-x-1/2 z-[5000] pointer-events-none " aria-live="polite" aria-atomic="true">
    <transition-group name="toast" tag="div" class="flex flex-col items-center gap-2">
      <div
        ...
        :key="n.id"
        class="bg-white dark:bg-gray-700 dark:border-gray-600 pointer-events-auto min-w-[280px] max-w-[90vw] rounded-xl px-4 py-3 border relative flex items-center justify-center"
      >
        <span :class="['absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full', dotClass(n.type)]"></span>
        <span class="block w-full text-sm font-medium text-gray-800 dark:text-gray-50 text-center">{{ n.message }}</span>
        <button
          class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-100 transition-colors duration-200"
          @click="remove(n.id)"
          aria-label="Sluiten"
        >
          <X class="w-4 h-4" />
