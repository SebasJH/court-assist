...
export default {
  ...
  setup(props, { emit }) {
    ...
    function addPlayer() {
      ...
      players.push({ id, x: 0.5, y: 0.5, number, color: '#111', role: 'offense', pos: String(number) }) // default black
      redraw()
    }
    ...
    function setPlayerRole(role){
      try {
        ...
        if (p) {
          let s = String(role||'offense').toLowerCase()
          if (s === 'bal') s = 'ball'
          else if (s === 'aanval') s = 'offense'
      ...
      players.forEach(p => {
        ...
        if (role === 'defense') {
          ...
          if (role === 'ball') {
            ...
            c.strokeStyle = col
            c.lineWidth = 2.5
            c.beginPath(); c.arc(x, y, 20.5, 0, Math.PI*2); c.stroke()
            c.restore()
          }
        }
      })
